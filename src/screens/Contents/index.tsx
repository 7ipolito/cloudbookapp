...
export function Contents({ navigation }: any) {
    ...
    function handleAddContent() {
        Alert.alert('Cloudbook', 'Escolha um tipo de conteúdo', [
            {
                text: 'Foto 🖼️',
                onPress: () => {
    ...
    async function listContents() {
        ...
        files.map(
            async (file: {
                ...
                const nameAnnotationFormatted = file.name.substring(
                    0,
                    file.name.indexOf('.txt')
                );
                const namePhotoFormatted =
                    file.name.substring(0, file.name.indexOf('.jpg')) ||
